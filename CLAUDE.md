...
├── index.py                      # IndexState + activation_floor (host-side index policy only;
│                                 #   the index itself lives in the kernel — facade retired, #355)
...
└── cli/
    ...
    ├── collection_cmd.py         # shrike collection query/prune/check/reload
...
│   ├── test_index_policy.py      # activation_floor math (the index itself is kernel-tested)
...
All three must pass cleanly. CI (`.github/workflows/test.yml`) is **gated on the `ci` label** — to keep CI lean, none of the test lanes run on a PR until it carries `ci` (or `rc`, which implies full CI for a release candidate), so a PR that churns through code review doesn't re-run the whole suite on every push. Add the `ci` label once review is complete to run CI on the final state. The gate lives on the `changes` job's `if` (the root of the dependency graph — every test lane `needs: changes`, so skipping `changes` cascades and not a single test runner spins up); `ci-ok` (the single required status check) **fails** with an actionable message until the label is present, so CI is *deferred, not skipped* — it must still pass before merge. The trigger types include `labeled`/`unlabeled` so toggling the label re-evaluates. When the label is present (Linux x64 only): a `lint` job and a `tests` job — ONE `bazel test` invocation over the full graph plus the `manual` embedding halves, with no per-suite path gating: Bazel's dependency analysis + the disk cache decide what re-executes, including test results (an unchanged target replays as "(cached) PASSED"; warm-cache.yml seeds main's scope with build *and* test results, #422). **Coverage is reported but never gated**, and lives in its own workflow (`.github/workflows/coverage.yml`, off the per-PR path entirely): it runs on `rc`-labelled PRs so a release candidate carries a fresh number, *and* 3x/week on `main` + on demand (`workflow_dispatch`) — each uploads an `htmlcov` artifact + a job-summary report and never fails on a threshold, and the main/scheduled runs publish the README coverage badge to the orphan `badges` branch. It is deliberately separate from `ci-ok` (it must not block a merge). The `fail_under` target is enforced only locally by `scripts/coverage.sh` — see the Coverage section above. macOS and ARM run the full integration suite via the `cross-platform` job, gated by **two labels with distinct intents**: `rc` (release candidate) selects the **full os matrix** — apply before tagging a release for complete cross-platform coverage; **`macOS`** selects **only the macos-latest leg** — apply on a PR whose change is macOS-specific (the Swift/Vision platform glue, the PyO3 link path) and needs a macOS CI proof without the broader rc treatment (it doesn't imply `ci` — a normal merge needs both labels). Neither leg ever runs on plain PRs or on merge to `main`. (The repo is public, so standard hosted runners — including macOS/ARM — bill at $0; the gating discipline is kept for wall-clock, queue, and signal-noise, and so the posture survives if the repo ever goes private, where macOS bills at 10×.) These lanes stay off the normal iterate-and-merge loop entirely. The PR trigger lists `labeled` in its `types` so adding any gate label re-triggers CI.
The bazel/cross-platform lanes **cache** the pinned llama-server and the GGUF test model (`actions/cache`) so they aren't re-downloaded every run. But an `actions/cache` entry is only restorable from the run's own branch or the **default branch** — and `test.yml` runs on PRs only, so nothing ever seeds `main`'s cache scope and every PR would cold-download the model from HuggingFace (which intermittently `429`s). A separate **cache-warmer** (`.github/workflows/warm-cache.yml`) closes that gap: it runs on `main` daily (plus on lock-changing merges and via `workflow_dispatch`) and executes the *same* `bazel test` invocation as the PR lane, so `actions/cache` saves the compiled graph, the **test results**, and the embedding externals into `main`'s scope, which every PR then restores from (it also warms the cargo-side `rust-lint`/`rust-coverage` caches). It uses the *same* composite/cache keys as the tests lane (`.github/actions/bazel-setup`); llama-server stays pinned via `scripts/llama-server.lock` and the model via the `EMBEDDING_MODEL_*` constants in `tests/integration/model_cache.py` (both bumped manually). The fixture's `download_with_retry` (backoff on `429`/5xx) remains the backstop for a cold/evicted run (#83, #93).

A separate **release workflow** (`.github/workflows/release.yml`) fires on `push` of a `v*` tag (not on PRs): it runs the same single `bazel test` invocation as the PR/rc lanes on all three platforms unconditionally (#422 — the old pip-lane release test job stopped working at the native cutover; harmonizing it onto Bazel also dropped the HuggingFace fetch from release day), builds the release artifacts — the **platform-tagged `shrike-mcp` wheels** (#497: cp312-abi3 with `shrike_native` inside, one per platform — macOS arm64/x86_64, linux x86_64/aarch64, the linux pair auditwheel-repaired to manylinux tags), the sdist (the version is stamped from the tag, so the build/test jobs check out with `fetch-depth: 0`), the `anki-cards.skill` bundle (`scripts/package-skill.py`), and a `SHA256SUMS` — and cuts a GitHub Release with them all attached. Release notes come from the matching `## [X.Y.Z]` section of `CHANGELOG.md` (falling back to auto-generated commit notes); a pre-release tag (`vX.Y.Z-rc.N`, detected by the SemVer hyphen) is published as a GitHub pre-release. A final (non-rc) release is also published to PyPI as `shrike-mcp` via trusted publishing — there is no separate `shrike-native` distribution (the extension ships inside the platform wheel; #340 records the plugin-extension-wheel idea for the day install-size pressure returns).
...
YAML at the platform config directory (`config.yml`). **User-managed: `shrike server start` never writes it unless `--save-config` is passed** (#56); with it, start persists the resolved *operational* flags; without it, start is a no-write operation and always reflects exactly the flags it was given.

**The capability sections are config-file-only (#498, docs/distribution.md).** `embedders:` (a list of vector-space entries: `modalities` + `runtime: onnx|remote`, with `endpoint`/`api_key_env`/`pooling`/`providers`/`batch_size` scoped per entry), `recognizers:` (keyed `ocr`/`asr`/`describe`), and `managed:` (`llama_server` with `manage: auto|attach|off`; `sync_server`, #36) have **no flag or env spelling** — structured config has one home. `shrike/profiles.py` is the model: `parse_capabilities` (parse/validate; inapplicable knobs are structural errors), `resolve_profile` (intersect with `shrike_native.build_features()` — an uncompiled runtime is a config error naming the build profile; an unwired capability names its issue: #229 multi-space, #485 asr/describe, #502 remote OCR), `plan_to_runtime_params` (the bridge onto `EmbeddingRuntime`'s param shape). The CLI validates at spec-build time and passes `--config` to the daemon, which re-resolves the file itself (structured entries can't ride flags); under a v2 config the legacy flags are rejected and the legacy env twins warn as ignored. `--save-config` passes the v2 sections through verbatim and never writes the bridged params back as a legacy `embedding:` section.

**The legacy cascade survives one release as warn-and-map (#523 is the removal).** A config *without* v2 sections runs the old resolution: config defaults → config values → env vars (`SHRIKE_URL`, `SHRIKE_COLLECTION`, the `SHRIKE_EMBEDDING_*` family, `LLAMA_SERVER_PATH`, `SHRIKE_CACHE_DIR`, `SHRIKE_INDEX_SAVE_DELAY`, `SHRIKE_INDEX_SAVE_THRESHOLD`, `SHRIKE_COOPERATIVE_LOCK`, `SHRIKE_LOCK_HOLD_SECONDS`, `SHRIKE_MEDIA_ALLOW_PRIVATE_FETCH`, `SHRIKE_PUBLIC_URL`, `SHRIKE_MEDIA_PATH_ROOTS`) → CLI flags, with a legacy `embedding:`/`recognition:` section warn-mapped onto the v2 model (`_migrate_legacy`). Operational settings (cache dir, index flush tuning, transport, locking) keep the cascade permanently via `config.resolve_cache_dir()` / `resolve_index_save()` / `resolve_transport()` / `resolve_locking()`. `save_config` persists `collection`, `cache_dir`, non-default `server.*`, legacy `embedding.*` (legacy setups only), and any set `index.save_delay` / `index.save_threshold`; **logging overrides are read from config but never written by `--save-config`** — set `logging.level` / `logging.dir` in `config.yml` directly. The `index.*` flush knobs and `cache_dir` resolve to `None` in config, meaning "use the server's built-in defaults" (the kernel saver's 60s/100 and the platform cache dir) — the numeric defaults live in `shrike.index`, not duplicated in config.
...
- **Pooling type:** `pooling: {mean|last|cls|none}` on the embedder entry (legacy: `--embedding-pooling`/`SHRIKE_EMBEDDING_POOLING`) is passed to llama-server as `--pooling`. Allowed only where Shrike launches the server producing the vectors (onnx entries; remote entries under `manage: auto`) — an external endpoint or an attached server owns its own pooling, and declaring it there is a ProfileError. Unset means "use the model's GGUF default" — fine for BERT-family models (`all-MiniLM-L6-v2`, `bge-m3`) that carry mean pooling in metadata. **Last-token models (Jina v5, Qwen3-Embedding) need `--embedding-pooling last`**: their pooling type isn't in the GGUF metadata, so without it llama-server defaults to mean and produces wrong embeddings (and some of these architectures may need a newer llama.cpp than the pinned `LLAMA_TAG` — see `scripts/llama-server.lock`). Pooling is folded into `model_id` (below) so changing it forces an index rebuild.
- **Generic arg passthrough:** `managed.llama_server.args` (a list of raw token strings; legacy: `--embedding-arg` repeatable / `embedding.extra_args` / `SHRIKE_EMBEDDING_ARGS` as one shlex string) appends raw tokens to the llama-server command for the long tail of **runtime-only** flags (`--flash-attn`, `--ubatch-size`, gpu split, …). Each entry is `shlex`-split at command-build time and appended last. Two guardrails: (1) Shrike-owned flags (`--model`/`-m`/`--host`/`--port`/`--embeddings`/`--embedding`, plus their value token) are stripped with a warning — `--host` especially, since llama-server is pinned to loopback (audit §1.1); (2) the effective passthrough is folded into `model_id`, so **any** change forces a rebuild (conservative — Shrike can't tell a vector-affecting flag from a perf-only one in an opaque bag). **Vector-affecting flags must be typed settings** (like `--embedding-pooling`), not buried here. Normalization is *not* such a setting: USearch's `cos` metric is scale-invariant (verified in the index-engine tests), so `--embd-normalize` is moot.
...
3. **Persistence** — the index is saved to disk on graceful shutdown (signal handler and `POST /shutdown`), at the end of a rebuild, and via a **debounced flush** during normal operation. The kernel's `DebouncedSaver` (`native/shrike-kernel/src/index_orchestrator.rs`) owns the debounce: every maintained write op requests a save, and the index is written either **`save_delay` seconds after the last change** (idle debounce, default 60s) **or immediately once `save_threshold` unsaved changes accumulate** (burst cap, default 100), whichever comes first. The flush rides the kernel runtime's own timers (`tokio::time`) and blocking pool — no asyncio involvement, and no fixed-interval polling — driven by edit activity. This bounds how much incremental work a hard kill / crash discards: once a flush lands and the server goes idle, the on-disk index (and its `col_mod`) are current, so it reloads without a rebuild. For edits since the last flush, the `col.mod` mismatch on next startup still triggers a full rebuild — correctness is preserved either way, at the cost of a re-embed. `save_delay`/`save_threshold` are configurable (config `index.*`, env, `--index-save-*` flags); the cache location is `cache_dir`/`SHRIKE_CACHE_DIR`/`--cache-dir`. (Tombstone compaction is unnecessary on the pinned USearch — see the index code comments.)
...
- **Then label, auto-merge, move on.** Apply `ci` once review findings are
  ...
  proactively move on to the next step. Labeling right after a push is safe:
  the workflow's concurrency group keys on the ci/rc label state (#400), so
  the label-less `synchronize` sentinel and the `labeled` full run land in
  different groups and can't cancel each other (the old race that needed a
  label toggle and burned a full re-run is gone).
